        ...
        if (-not $completed) {
            ...
            if (-not $process.WaitForExit(5000)) {
                return [pscustomobject]@{
                    ...
                    StandardOutput = ''
                    StandardError = if ($terminationError) {
                        "The timed-out worker could not be terminated: $terminationError"
                    }
                    else {
                        'The timed-out worker did not exit within the 5-second termination grace period.'
                    }
                }
            }
        }
        else {
            # Ensure redirected asynchronous output has been fully drained.
            $process.WaitForExit()
        }
...
function Invoke-PscxAnalyzerWorkItem {
    ...
    for ($attempt = 1; $attempt -le $MaximumAttempts; $attempt++) {
        ...
        if ($processResult.TimedOut) {
            $reason = "Analyzer worker exceeded the $($processResult.TimeoutSeconds)-second timeout."
        }
        ...
        $failures.Add([pscustomobject]@{
                ...
                TimeoutSeconds = $processResult.TimeoutSeconds
...
Timeout seconds: $($item.TimeoutSeconds)
Reason: $($item.Reason)
Standard output:
$standardOutput
Standard error:
...
function Get-PscxCanonicalAnalyzerDiagnostic {
    [CmdletBinding()]
    param(
    ...
    $canonical = foreach ($item in $Diagnostic) {
        if ($null -eq $item) {
            continue
        }
        $relativePath = [IO.Path]::GetRelativePath(
            $root,
            [IO.Path]::GetFullPath([string] $item.ScriptPath)
        ).Replace('\', '/')
    ...
    @($canonical | Sort-Object Severity, RelativePath, Line, RuleName, Message)
}
...
Export-ModuleMember -Function `
    ...
    Get-PscxCanonicalAnalyzerDiagnostic

# SIG # Begin signature block
# MIInmgYJKoZIhvcNAQcCoIInizCCJ4cCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCXchRWhq/CoOol
# x1HnSkYs8I6dwILNBZzKzQWo4cDrvaCCIHEwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# MQ4wDAYKKwYBBAGCNwIBFjAvBgkqhkiG9w0BCQQxIgQg0c8TiidNCtWZ10sMnwIp
# E+UJs5Lj74MnGoo9RlUki2kwDQYJKoZIhvcNAQEBBQAEggIAj0k4RX5UjJnxJORv
# yKboQVknt6jpPYSQ4hJXsgbLyPyjio9KMLjfSumoCZ8v5R7yfqgXabCu0H0U8kjF
# JCBraXGITutZptIZHr4QF+OysXhmncAhyunICmgm9dgWci/hcYfFCuIF7Tw3oHjh
# FND9GCR65BuWu+Io8QnpZfpo5RoNOjHeqESU370XYuL2MuZ6ur2ZPdeEjNFTliDs
# l2rVC+jlNu8klvxrCXbAihcWIeOvNWtLG2w2IziQhFz0aXzlCyLJDD5esYnM8Stb
