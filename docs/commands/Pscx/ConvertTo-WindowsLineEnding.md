...
title: ConvertTo-WindowsLineEnding
---
...
```
ConvertTo-WindowsLineEnding [-Path] <PscxPathInfo[]> [[-Destination] <string>]
...
```
ConvertTo-WindowsLineEnding [-LiteralPath] <PscxPathInfo[]> [[-Destination] <string>]
 [[-Encoding] <StringEncodingParameter>] [-Force] [-NoClobber] [-FinalNewline <FinalNewlineMode>]
 [-Check] [-WhatIf] [-Confirm] [<CommonParameters>]
```
...
Converts CRLF, LF, and CR line endings to Windows CRLF (`\r\n`) line endings.
...
```powershell
Get-Help ConvertTo-WindowsLineEnding -Full
```
...
```powershell
ConvertTo-WindowsLineEnding -Path ./src/*.cs -Check |
    Where-Object NeedsConversion
```

Returns a structured result for each matching file and filters the results to
files that need Windows line endings. No files are written.

### Example 3 - Convert a file and remove its final newline

```powershell
ConvertTo-WindowsLineEnding -LiteralPath ./input.txt -Destination ./output.txt -FinalNewline Remove
```

Writes `output.txt` with CRLF line endings and no final newline while preserving
the source encoding and byte-order mark.
